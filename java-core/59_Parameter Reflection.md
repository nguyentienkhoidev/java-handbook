...
Lấy tên tham số (chỉ chính xác khi biên dịch với `-parameters`).
...
Lấy generic type (nếu có).

`isNamePresent()`

Kiểm tra xem tên tham số có sẵn không.

`isVarArgs()`

Kiểm tra tham số có phải varargs không (`...`).

`isFinal()`

Kiểm tra tham số có modifier `final` không.

`getAnnotations()`

Lấy tất cả annotation gắn trên tham số.
